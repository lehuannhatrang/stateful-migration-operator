...
```bash
# Navigate to the multi-cluster deploy directory
cd deploy/multi-cluster-karmada
...
sed -i 's|YOUR_DOCKERHUB_USERNAME/stateful-migration-operator:latest|yourusername/stateful-migration-operator:latest|g' deploy/multi-cluster-karmada/all-in-one.yaml

# 3. Apply to mgmt-cluster
kubectl apply -f deploy/multi-cluster-karmada/all-in-one.yaml
...
```bash
# Apply in order
kubectl apply -f deploy/multi-cluster-karmada/namespace.yaml
kubectl apply -f deploy/multi-cluster-karmada/rbac.yaml
kubectl apply -f deploy/multi-cluster-karmada/deployment.yaml  # Edit image first!
kubectl apply -f deploy/multi-cluster-karmada/service.yaml
```
...
kubectl delete -f deploy/multi-cluster-karmada/all-in-one.yaml
